$(document).ready(function() {
	$('ul[name="address"] a.nav-link').click(function() {
		setTimeout(function() {
			refreshPreviewMail()
		}, 200);
	});
...
function save_or_send(status, mail) {
	if (verifForm()) {
		$('div[name="save"], div[name="send"]')
		...
		var attachments = {};
		var i = 0;
		$('table[name="attachments"] tr.attachment').each(function() {
			var attachment = {};
			attachment.filename = $(this).attr('data-file');
			attachment.name = $(this).attr('data-name');
			attachment.nbPages = $(this).attr('data-nbPages');
		...
		$.ajax({
			...
			success: function(response) {
				$('.modal[name="save_result"] h5').html(response.title);
				$('.modal[name="save_result"] .modal-body').html(response.message);
				$('.modal[name="save_result"]').modal('show');
				$('.modal[name="save_result"]').on('hide.bs.modal', function(e) {
					location.href = response.redirection;
				});
			},
			error: function(response) {
				$('.modal[name="save_result"] .modal-dialog').addClass('modal-lg');
				$('.modal[name="save_result"] .modal-header').addClass('alert-danger');
				$('.modal[name="save_result"] h5').html('Une erreur est survenue (code ' + response.status + ')');
				$('.modal[name="save_result"] .modal-body').html(response.responseText);
				$('.modal[name="save_result"]').modal('show');
			}
		});
	} else {
		$('.modal[name="save_result"] .modal-dialog').addClass('modal-lg');
		$('.modal[name="save_result"] .modal-header').addClass('alert-warning');
		$('.modal[name="save_result"] h5').html('Un ou plusieurs champ(s) ne sont pas correct(s)');
		$('.modal[name="save_result"] .modal-body').html('Un ou plusieurs champ(s) ne sont pas correct(s). <br />Les champs présentant des erreurs s\'affichent en rouge.<br /><br />Vérifiez la longueur (pas plus de 38 caractères par ligne d\'adresse).<br />Vérifiez les champs requis.');
		$('.modal[name="save_result"]').modal('show');
		$('.modal[name="save_result"]').on('hide.bs.modal', function(e) {
			$('.modal[name="save_result"] .modal-header').removeClass('alert-warning');
		});
	}
}
...
function verifForm() {
	...
	if ($('a[href="#tab-company"]').hasClass('active')) {
		...
		if ($('input[name="raison_sociale"]').val().length > 0 && $('input[name="raison_sociale"]').val().length <= 38) {
			$('input[name="raison_sociale"]').addClass('is-valid');
		} else {
			$('input[name="raison_sociale"]').addClass('is-invalid');
		}

		if ($('input[name="service"]').val().length > 0 && $('input[name="service"]').val().length <= 38) {
		...
		if (($('select[name="civilite"]').val().length + $('input[name="nom"]').val().length + $('input[name="prenom"]').val().length) <= 38 && $('input[name="nom"]').val().length > 0 && $('input[name="prenom"]').val().length > 0) {
		...
		if (($('input[name="appartement"]').val().length + $('input[name="etage"]').val().length + 5) <= 38 && $('input[name="appartement"]').val().length > 0) {
	...
	if ($('input[name="adresse"]').val().length <= 38 && $('input[name="adresse"]').val().length > 0) {
	...
	if (($('input[name="code_postal"]').val().length + $('input[name="ville"]').val().length) <= 38 && $('input[name="code_postal"]').val().length > 0 && $('input[name="ville"]').val().length > 0) {
